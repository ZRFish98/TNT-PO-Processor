...
def _load_queue() -> pd.DataFrame:
    ...
    try:
        ...
        st.error(t("queue.db_error", error=e))
        return pd.DataFrame()
...
def render(settings: dict):
    ...
    if not test_connection():
        st.error(t("queue.no_db"))
        return

    # ── Region filter ─────────────────────────────────────────────────────────
    region_col, refresh_col, info_col = st.columns([2, 1, 3])
    with region_col:
        region_options = [t("queue.region.all"), t("queue.region.east"), t("queue.region.west")]
    ...
    with refresh_col:
        auto_refresh = st.toggle(t("queue.auto_refresh"), value=False, key="queue_auto_refresh")
    with info_col:
    ...
    if auto_refresh:
        try:
            ...
            st.warning(t("queue.no_autorefresh"))

    # ── Queue table ────────────────────────────────────────────────────────────
    queue_df = _load_queue()

    # Apply region filter
    if region_filter == t("queue.region.east") and "region" in queue_df.columns:
        queue_df = queue_df[queue_df["region"] == "east"]
    elif region_filter == t("queue.region.west") and "region" in queue_df.columns:
    ...
    if queue_df.empty:
        ...
        with queue_col:
            st.subheader(t("queue.header", count=len(queue_df)))
            select_all = st.checkbox(t("queue.select_all"), key="queue_select_all")
            ...
            queue_df["Status"] = queue_df["status"].apply(
                lambda s: f"\U0001f535 {s}" if s == "unprocessed"
                else (f"\U0001f7e1 {s}" if s == "processing"
            ...
            if "region" in queue_df.columns:
                queue_df["Region"] = queue_df["region"].map(
                    {"east": t("queue.region_display.east"), "west": t("queue.region_display.west")}
                ).fillna("\u2014")
            else:
                queue_df["Region"] = "\u2014"
            ...
            edited = st.data_editor(
                ...
                height=500,
                column_config={
                    "Select": st.column_config.CheckboxColumn(t("queue.col.select"), width="small"),
                    "id": st.column_config.NumberColumn(t("queue.col.id"), width=50),
                    "Status": st.column_config.TextColumn(t("queue.col.status"), width="medium"),
                    "Pages": st.column_config.NumberColumn(t("queue.col.pages"), width=40, format="%d"),
                    "Downloaded": st.column_config.TextColumn(t("queue.col.downloaded"), width="small"),
                    "Received": st.column_config.TextColumn(t("queue.col.received"), width="small"),
                    "store_id": st.column_config.NumberColumn(t("queue.col.store"), width="small"),
                    "Region": st.column_config.TextColumn(t("queue.col.region"), width="small"),
                    "order_date": st.column_config.TextColumn(t("queue.col.order_date"), width="small"),
                    "delivery_date": st.column_config.TextColumn(t("queue.col.delivery_date"), width="small"),
                    "original_filename": st.column_config.TextColumn(t("queue.col.filename")),
                    "source": st.column_config.TextColumn(t("queue.col.source"), width="small"),
                },
            ...
            if selected_ids:
                ...
                if invalid_count:
                    st.warning(t("queue.skip_processed", count=invalid_count))

            btn_col, stat_col = st.columns([1, 3])
            with btn_col:
                process_disabled = len(valid_ids) == 0
                if st.button(
                    t("queue.btn.process", count=len(valid_ids)),
                    type="primary",
                    disabled=process_disabled,
                ):
                    with st.spinner(t("queue.spinner.loading")):
                        extracted_df = _fetch_extracted_data(valid_ids)

                    if extracted_df.empty:
                        st.error(t("queue.err.no_data"))
                    else:
            ...
            with del_col1:
                if st.button(
                    t("queue.btn.delete", count=len(selected_ids)),
                    ...
                    with get_db_connection() as conn:
                        ...
                        conn.commit()
                    st.success(t("queue.msg.deleted", count=len(selected_ids)))
                    st.rerun()
            with del_col2:
                if st.button(t("queue.btn.clear_processed")):
                    with get_db_connection() as conn:
                        with conn.cursor() as cur:
                            cur.execute(
                        ...
                        conn.commit()
                    st.success(t("queue.msg.cleared"))
                    st.rerun()
            with dl_col:
                if selected_ids:
                    ...
                    if zip_data:
                        downloaded = st.download_button(
                            t("queue.btn.download", count=len(selected_ids)),
                            data=zip_data,
                        ...
                        st.button(
                            t("queue.btn.download_disabled"),
                            disabled=True,
                            help=t("queue.help.no_pdf"),
                        )

        # ── PDF Preview panel ─────────────────────────────────────────────────
        with preview_col:
            st.subheader(t("queue.preview.title"))
            ...
            if not preview_options:
                st.caption(t("queue.preview.empty"))
            else:
                selected_label = st.selectbox(
                    t("queue.preview.select"),
                ...
                if pdf_data:
                    ...
                    st.info(t("queue.preview.no_pdf"))

    st.divider()

    # ── Manual PDF Upload ──────────────────────────────────────────────────────
    st.subheader(t("queue.upload.title"))
    st.caption(t("queue.upload.caption"))

    uploaded_files = st.file_uploader(
        t("queue.upload.label"),
        type=["pdf"],
    ...
    if uploaded_files:
        if st.button(t("queue.upload.btn")):
            new_ids = []
            all_errors = []
            with st.spinner(t("queue.upload.spinner")):
            ...
            st.success(t("queue.upload.success", count=len(new_ids)))

            if all_errors:
                with st.expander(t("queue.upload.warnings")):
                    for e in all_errors:
                        st.write(e)
